...
## はじめに
このファイルではレイトレ合宿10のルールや提出物の仕様を説明しています。**各自熟読をお願いします。**

複数インスタンス実行の方法やサンプルプログラムについても説明しています。
...
| - | - | - | - | - | - |
| [g5.xlarge](https://aws.amazon.com/jp/ec2/instance-types/g5/) x1 | AMD EPYC 7R32<br>4 vCPU | 16 GiB | [NVIDIA A10G](https://www.nvidia.com/ja-jp/data-center/products/a10-gpu/) | EBS | 250 GiB NVMe SSD |
...
#### 複数インスタンス
今回はCPUインスタンスのみ複数インスタンスを使用します。
実行ファイルやアセットは各インスタンスに対して予め所定のパスに運営側で転送しておきますが、実行はひとつのインスタンスから始めます。この実行を開始するインスタンスをプライマリー(インスタンス)、2つ目のインスタンスをセカンダリー(インスタンス)と呼ぶことにします。\
...
ケース1に関しては何も難しいことは考えることはありませんが、当然計算リソースがインスタンス1台分になります。ケース2やケース3ではインスタンス間でプログラムの起動(やデータの転送)を行う必要があります。運営側で予めインスタンス間のSSH通信のセットアップを行っておきます。またケース3は実行ファイル間の直接通信のためにSSHとは異なるポートを使う必要があるため、各インスタンスともにファイアウォールにおけるポート12345の許可を設定しておきます。

### 実行

提出物内の `<root directory>` を各インスタンスのホームディレクトリに配置します。すなわち `run.ps1` は次の絶対パスに配置されることになります。\
...
1. 作成したセキュリティグループが表示されるので「インバウンドのルールを編集」をクリック。
   1. 「ルールを追加」を押し、\
      タイプ: "すべてのトラフィック", ソース: "カスタム" でソースのテキストボックスをクリックして自身のセキュリティグループを選ぶ。
   1. 「ルールを保存」をクリック。
